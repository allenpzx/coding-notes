...
2. chatroom 实例的增删改(点击右侧联系人，关闭一个 chatroom)
...
- 将`text/image`类型的消息通过 websocket 或者 http 发送给 server，假如 client A 向 client B 发送一条消息 MSG_A，那么将发生 👇
  ...
  6. client 收到消息之后, 如果消息没有`message_id`则直接推入聊天数组，如果有`message_id`则把这条消息比如 MSG_A 找到，并检查这条消息的`message_status === pending`则设置 `message_status = success`, 如果这条消息`message_status === error` 则不做重制
...
4. stacking context. z-index 是相对的. 相邻兄弟 A 和 B 元素 z-index: 100;，其中 A 元素的子元素 a1 如果 z-index: 999; 宽高足够大 也不会遮挡 B， 所以在包或者写 Modal 或者 notification 组件的时候可以放到最外层用 store 控制， 或者用 portal 传送到组件的外层，防止干扰
...
```
6. React SyntheticEvent `Event Pooling`
