...
class ChatMain extends React.Component<ContainerProps, ContainerState> {
    ...
    try {
      ...
      if (status === 200 && data) {
        ...
        results = results.filter(
          (v: MessageProps) =>
            v.msg_type && message_display_type.includes(v.msg_type as number)
        );
  ...
  onRoomMessage = (data: any) => {
    ...
    if (
      msg_type === 8 &&
      this.props.user.userInfo.userId === _get(data, ["receiver_id"])
    ) {
      this.setState(prev => ({ results: prev.results.concat(data) }));
    }
    if (msg_type === 7 || msg_type === 9) {
      this.setState(prev => {
        ...
        for (let i = next.length - 1; i > 0; i--) {
          if (
            next[i]["msg_type"] === 6 &&
            next[i]["id"] === _get(data, ["id"])
          ) {
            next[i][
              "extra"
            ].offer_status = mapMessageTypeToOfferBubbleStatus.get(
              msg_type.toString()
            );
            break;
          }
  ...
  addTimeMessage = (results: MessageProps[]): MessageProps[] => {
    return results.reduce((prev, curr: any, index: number) => {
      const left =
        index === 1 && !this.state.next
          ? 0
          : _get(prev.slice(-1)[0], ["created_at"]);
